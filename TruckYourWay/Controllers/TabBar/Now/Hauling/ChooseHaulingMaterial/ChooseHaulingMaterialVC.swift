//
//  ChooseHaulingMaterialVC.swift
//  TruckYourWay
//
//  Created by Anil Choudhary on 28/02/19.
//  Copyright © 2019 Samradh Agarwal. All rights reserved.
//

import UIKit

class ChooseHaulingMaterialVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if Constant.MyVariables.appDelegate.truckCartModel != nil {
            ...
            if let arrayData = Constant.MyVariables.appDelegate.getResponseDataStructFromUserDefault(forKey: "arrMaterialListTemp") {
                self.arrSelectedMaterialResponse = arrayData

                guard let materialVC = (self.navigationController!.viewControllers[(self.navigationController!.viewControllers.count)-2]) as? HaulingMaterialVC else { return }
        ...
        arrSelectedMaterialResponseTemp = [self.arrSelectedMaterialResponse[self.selectedIndex]]
    ...
    @objc func appIsGoingInBackground() {
        ...
        guard let materialVC = (self.navigationController!.viewControllers[(self.navigationController!.viewControllers.count)-2]) as? HaulingMaterialVC else { return }
        Constant.MyVariables.appDelegate.saveResponseDataStructToUserDefault(self.arrSelectedMaterialResponse, forKey: "arrMaterialListTemp")
    ...
    @IBAction func actionBack(_ sender: Any) {
        ...
        guard let materialVC = (self.navigationController!.viewControllers[(self.navigationController!.viewControllers.count)-2]) as? HaulingMaterialVC else
        {
            return
        }
        materialVC.arrMaterialListTemp[self.selectedIndex] = dddd
        //        materialVC.delegateOnPop = (self as! ChooseMaterialDelegate)
...
//        let needRemaining = 5 - remider
        ...
        {
            self.showAlert("Cannot proceed with empty cart.")
            return
        }
...
//        }
    ...
    {
        let row = sender.row!
        let deliveryObj =  self.arrSelectedMaterialTemp[row]
        ...
        else
        {
            ...
            self.arrSelectedMaterialTemp[row] = deliveryObjTemp
    ...
    func appendData(arr: NSArray) -> [DeliveryData]
    {
        ...
        for item in arr
        {
            ...
            delivery = DeliveryData(id:dict.value(forKey: "id") as? Int ?? 0,
                                    ...
                                    status:dict.value(forKey: "status") as? String ?? "", dis:dict.value(forKey: "distance") as? String ?? "",
    ...
    func updateDataAfterGetMaterialByLocation(arrDeliveryMaterial:[DeliveryResponse],selectedItemIndex:Int,flag:Int)
    {
        ...
        for item in arrNew {
            ...
            if(i != selectedItemIndex)
            {
                ...
                arrOld[i].status = item.status //To change the mterial count to 0
            }
        ...
        deliveryObjTemp = DeliveryData(id: deliveryObj.id, quantity:updatedQuantity, size: deliveryObj.size, category:deliveryObj.category, material_image: deliveryObj.material_image, version_no: deliveryObj.version_no, status: "", dis: deliveryObj.distance, uid: deliveryObj.uuid)
        self.arrSelectedMaterialTemp[selectedTagofItem] = deliveryObjTemp
        Constant.MyVariables.appDelegate.saveTrucCartData()
...
extension ChooseHaulingMaterialVC: UITableViewDelegate, UITableViewDataSource
{
...
//        } else {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblLoads.text = (self.arrSelectedMaterialTemp[indexPath.row].category).uppercased() == "DIRT" ? "LOADS" : "LOADS"
...
extension ChooseHaulingMaterialVC: UITextFieldDelegate
{
